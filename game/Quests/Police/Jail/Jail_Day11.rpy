label jail_day11:
    ...
    call textonblack(_("ДЕНЬ 11")) from _call_textonblack_17
    img black_screen
    with Dissolve(1)
    music Jail_Clock
...
label jail_day11_Bed(obj_name, obj_data):
    if act == "l":
        call jail_day1_Bed(obj_name, obj_data) from _call_jail_day1_Bed_5
        return
    ...
    if jailDaySceneStage == 3:
        ...
        if jailCageBlackmailEnded == True:
            call jail_day12() from _call_jail_day12
            return
        stop music fadeout 1.0
        ...
        mt "(хмык)"
        call jail_day12() from _call_jail_day12_1
    return
...
label jail_day11_Cage(obj_name, obj_data):
    ...
    if act == "l":
        call jail_cage_blackmail_day() from _call_jail_cage_blackmail_day_2
        call refresh_scene_fade() from _call_refresh_scene_fade_101
        return
    if act == "w":
        $ cageInteractLabel = "jail_day11_Cage_Interact"
        call change_scene("police_jail_cage_scene") from _call_change_scene_192
        return
