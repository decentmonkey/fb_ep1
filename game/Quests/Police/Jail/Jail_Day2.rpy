...
label jail_day2_Bed(obj_name, obj_data):
    if act == "l":
        call jail_day1_Bed(obj_name, obj_data) from _call_jail_day1_Bed_2
        return
    if jailDaySceneStage == 0:
    ...
    if jailDaySceneStage == 4:
        menu:
            ...
            "Не ложиться":
                return
        call jail_day3() from _call_jail_day3
        return
    return
...
label jail_day2_Cage(obj_name, obj_data):
    if jailCageViewed == False:
        ...
        call jail_cage() from _call_jail_cage_2
        $ jailCageViewed = True
        call refresh_scene_fade() from _call_refresh_scene_fade_61
        return
    if act == "l":
        call jail_cage() from _call_jail_cage_3
    ...
    if act == "w":
        ...
        call change_scene("police_jail_cage_scene") from _call_change_scene_141
        return
