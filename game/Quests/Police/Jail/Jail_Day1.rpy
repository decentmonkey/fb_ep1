...
label jail_day1:
    ...
    call jail_day1_1 from _call_jail_day1_1
    return

label jail_day1_1:
    music I_Feel_You
    call change_scene("police_jail_scene", "Fade_long", False) from _call_change_scene_197
    return
...
label jail_day1_Monica(obj_name, obj_data):
    if jailDaySceneStage == 0:
        $ jailDaySceneStage = 1
        call jail_day1_2() from _call_jail_day1_2_1
        return
...
label jail_day1_Bed(obj_name, obj_data):
    if jailDaySceneStage == 2:
        if act == "h":
            ...
            img 2231
            "Пока, Маркус!"
            call jail_day2() from _call_jail_day2
            return
