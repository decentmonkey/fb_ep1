label jail_day5:
    ...
    call textonblack(_("ДЕНЬ 5")) from _call_textonblack_10
    img black_screen
    with Dissolve(1)
    music Jail_Clock
...
label jail_day5_Bed(obj_name, obj_data):
    if act == "l":
        call jail_day1_Bed(obj_name, obj_data) from _call_jail_day1_Bed_4
        return
    if jailDaySceneStage == 0:
        mt "Я.. хочу.. есть..."
    if jailDaySceneStage == 1:
        menu:
            ...
            "Не ложиться":
                return
        call jail_day6() from _call_jail_day6
        return
    return

label jail_day5_Cage(obj_name, obj_data):
    if act == "l":
        call jail_cage() from _call_jail_cage_5
        call refresh_scene_fade() from _call_refresh_scene_fade_77
        return
    if act == "w":
        $ cageInteractLabel = "jail_day5_Cage_Interact"
        call change_scene("police_jail_cage_scene") from _call_change_scene_158
        return
...
label jail_day5_Cage_Interact:
    if jailDaySceneStage == 0:
        call jail_day5_1() from _call_jail_day5_1
    ...
    if jailDaySceneStage == 1:
        ...
        if jailBoobsForFoodShowed == True:
            call jail_cage() from _call_jail_cage_6
            return
        return
    return
...
label jail_day5_1:
    ...
    if jailBoobsForFood == True:
        ...
        menu:
            "Сказать как его зовут." if monicaKnowOverseerName == True:
                call jail_boobs_for_food_end() from _call_jail_boobs_for_food_end_2
            "Показать грудь.":
                img 2373
                m "Стойте!"
                # показывает сиськи
                call jail_boobs_for_food() from _call_jail_boobs_for_food_3
            "Не показывать.":
    ...
    call change_scene("police_jail_food_scene", "Fade", False) from _call_change_scene_159
    return
...
label jail_day5_1a:
    ...
    call jail_day6() from _call_jail_day6_1
