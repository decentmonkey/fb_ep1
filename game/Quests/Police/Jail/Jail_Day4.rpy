...
label jail_day4_Bed(obj_name, obj_data):
    if act == "l":
        call jail_day1_Bed(obj_name, obj_data) from _call_jail_day1_Bed_8
        return
    if jailDaySceneStage == 0:
        mt "Черт побери! Я снова хочу есть!"
    if jailDaySceneStage == 1:
        menu:
            ...
            "Не ложиться":
                return
        call jail_day5() from _call_jail_day5
        return
...
label jail_day4_Cage(obj_name, obj_data):
    if act == "l":
        call jail_cage() from _call_jail_cage_7
        call refresh_scene_fade() from _call_refresh_scene_fade_163
        return
    if act == "w":
        $ cageInteractLabel = "jail_day4_Cage_Interact"
        call change_scene("police_jail_cage_scene") from _call_change_scene_252
        return
...
label jail_day4_Cage_Interact:
    if jailDaySceneStage == 0:
        call jail_day4_1() from _call_jail_day4_1
        return
    if jailDaySceneStage == 1:
        img 2253
        with fadelong
        ...
        "Мне не видно отсюда!"
        if jailBoobsForFoodShowed == True:
            call jail_cage() from _call_jail_cage_8
            return
        return
...
label jail_day4_1:
    ...
    if jailBoobsForFood == False:
        ...
        call change_scene("police_jail_food_scene", "Fade", False) from _call_change_scene_253
        return
    ...
    if jailBoobsForFood == True:
        ...
        "..."
        # показывает
        call jail_boobs_for_food() from _call_jail_boobs_for_food_6
        ...
        mt "Спасибо, жирный урод!"
        $ jailFoodInteractLabel = "jail_day4_1a"
        call change_scene("police_jail_food_scene", "Fade", False) from _call_change_scene_254
        return
