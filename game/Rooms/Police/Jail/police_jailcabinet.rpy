...
label police_jailcabinet_teleport(obj_name, obj_data):
    if obj_name == "Teleport_Jail_Cabinet2":
        call change_scene("police_jailcabinet2") from _call_change_scene_214
        return
    if obj_name == "Teleport_Jail_Cabinet":
        $ jailCabinetMonicaStanding = True
        call change_scene("police_jailcabinet") from _call_change_scene_215
        return
...
#        "Как мне выбраться отсюда???"
    if obj_name == "Chair":
        ...
        if act == "h":
            ...
            call change_scene("police_jailcabinet", "Fade", False) from _call_change_scene_216
            return
    if obj_name == "Lamp":
...
#                $ bitchmeterValue = 1000 #debug
                label police_jailcabinet_menu_loop1:
                    menu:
                        "Ты мерзкий ублюдок! Жалкое существо!!! (наказание)" if bitchmeterValue > maxBitchness / 2:
                            call jail_cabinet_punishment() from _call_jail_cabinet_punishment
                            return
            ...
            if jailCabinetState == 0:
                call jail_cabinet_dialogue2() from _call_jail_cabinet_dialogue2
                return
            if jailCabinetState == 1:
                call jail_cabinet_dialogue3() from _call_jail_cabinet_dialogue3
                return
            if jailCabinetState == 2:
                call jail_cabinet_dialogue4() from _call_jail_cabinet_dialogue4
                return
            if jailCabinetState == 3:
                call jail_cabinet_dialogue5() from _call_jail_cabinet_dialogue5
                return
            if jailCabinetState == 4:
                call jail_cabinet_dialogue6() from _call_jail_cabinet_dialogue6
                return
            if jailCabinetState == 5:
                call jail_cabinet_dialogue7() from _call_jail_cabinet_dialogue7_1
                return
